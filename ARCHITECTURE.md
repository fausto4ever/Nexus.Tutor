...
- `services/location.js`: frontera para `navigator.geolocation`. La meta es que ningún feature nuevo toque directamente la API nativa.
- `app.js`: controlador del recorrido 0.1.9 durante la migración. Debe ir perdiendo responsabilidades, no ganarlas.
...
4. `services/location.js` será el único propietario de geolocalización cuando termine la migración.
...
`app.js` todavía accede directamente a geolocalización y configuración. La existencia de `core/runtime.js` y `services/location.js` crea las fronteras necesarias para mover esas responsabilidades en fases pequeñas y comprobables.
