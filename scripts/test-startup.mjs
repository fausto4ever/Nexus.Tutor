...
const expected=['config.js','core/runtime.js','services/location.js','app.js','features/destinations.js','ui/shell.js'];
...
function makeClassList(owner){
  ...
  return{
    add(...items){items.forEach(item=>values.add(item));sync();},
    remove(...items){items.forEach(item=>values.delete(item));sync();},
    toggle(item,force){const next=force===undefined?!values.has(item):Boolean(force);if(next)values.add(item);else values.delete(item);sync();return next;},
...
function makeElement({id='',className='',dataset={}}={}){
  ...
  const el={id,className,dataset:{...dataset},value:'',disabled:false,checked:false,hidden:false,textContent:'',innerHTML:'',style:{},attributes:{},listeners,
    setAttribute(name,value){this.attributes[name]=String(value);if(name==='tabindex')this.tabIndex=String(value);},
    getAttribute(name){return this.attributes[name]??null;},
    addEventListener(name,fn,options){(listeners[name]??=[]).push({fn,capture:options===true||options?.capture===true});},
    focus(){document.activeElement=this;},setSelectionRange(){},showModal(){this.open=true;},close(){this.open=false;},appendChild(){},remove(){}}
...
const document={
  visibilityState:'visible',activeElement:null,documentElement:{dataset:{theme:'light'}},body:{appendChild(){}},
  querySelector(selector){
    if(selector.startsWith('#'))return elements.get(selector.slice(1))||null;
    if(selector==='meta[name="theme-color"]')return meta;
    return null;
  },
  querySelectorAll(selector){if(selector==='.nav-btn')return navButtons;if(selector==='.tab-content')return tabPanels;return[];},
...
const navigator={
  onLine:true,
  geolocation:{watchPosition(success,error){watchSuccess=success;watchError=error;return 1;},clearWatch(){},getCurrentPosition(){}},
  serviceWorker:{register:async()=>({})}
};
const windowListeners={};
const context={
  console:{log(){},warn(){},error(){}},document,navigator,localStorage,
  setInterval(){return 1;},clearInterval(){},setTimeout(){return 1;},clearTimeout(){},
  fetch:async()=>({ok:false,status:503,json:async()=>({})}),Blob:globalThis.Blob,URL:globalThis.URL,Date,Math,JSON,Number,String,Boolean,Array,Object,Set,Map,Promise,Error,
...
assert(typeof watchSuccess==='function'&&typeof watchError==='function','app.js no registró watchPosition');
...
console.log(`Startup: ${scriptOrder.length} scripts cargados, ${ids.size} IDs reales, 4 pestañas enlazadas.`);
